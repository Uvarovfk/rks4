def S(fi: float, o: float, y: float) -> List[List[float]]:
    ...
    S = [[np.cos(fi) * np.cos(y) + np.sin(fi) * np.sin(o) * np.sin(y),
          -np.sin(fi) * np.cos(y) + np.cos(fi) * np.sin(o) * np.sin(y), np.cos(o) * np.sin(y)],
         [np.sin(fi) * np.cos(o), np.cos(fi) * np.cos(o), np.sin(o)],
         [np.cos(fi) * np.sin(y) - np.sin(fi) * np.sin(o) * np.cos(y), -np.sin(fi) * np.sin(y) - np.cos(fi) * np.sin(o),
          np.cos(o) * np.cos(y)]]
